...
window.SKIN = (function () {
  ...
  function light(step) {
    ...
    // Камера отъезжает ровно, без замедления: пара уменьшается на каждом ответе и к концу уже далеко
    out.cam = mix(1.55, 1.0, k);
    // К восьмому вопросу кадр уже поднят к вершине, дальше держим небо
    out.posY = mix(76, 34, Math.min(1, s / 8));
    return out;
  }
